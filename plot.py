...
rt_cache_o3 = read_results("data_2048/results_cache_efficient_gcc_O3.bin")[:LAST_IND]
# rt_asm = read_results("data_1024/results_asm_matrix_mul.s.bin")[:LAST_IND]
rt_asm_simd = read_results("data_2048/results_simd_matrix_mul.s.bin")[:LAST_IND]
...
# ax.plot(input_sizes, rt_gcc_o0, color=colors[0], alpha=0.5, label='GCC -O0')
# ax.plot(input_sizes, rt_cache_o0, color=colors[0], label='GCC -O0 cache efficient')
# ax.plot(input_sizes, rt_gcc_o3, color=colors[1], alpha=0.5, label='GCC -O3')
